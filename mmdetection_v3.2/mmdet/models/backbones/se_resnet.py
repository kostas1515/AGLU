...
class UniRect_static(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        # input.grad = lambda_param.grad = kappa_param.grad = None
        # input_shifted = kappa_param*input -torch.log(lambda_param)
...
class UniRect(nn.Module):
    ...
    def __init__(self, num_parameters: int = 1,device=None, dtype=None) -> None:
        ...
        super().__init__()
        lambda_param = torch.nn.init.uniform_(torch.empty(num_parameters, **factory_kwargs),a=0.0,b=1.0)
        kappa_param = torch.nn.init.uniform_(torch.empty(num_parameters, **factory_kwargs),a=1.0,b=1.3)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        lambda_param = torch.clamp(self.lambda_param,min=0.0001)
        kappa_param = self.kappa_param
        p = torch.exp((1/lambda_param) * self.softplus(kappa_param*input -torch.log(lambda_param)))
        out = input * p
...
class Bottleneck(BaseModule):
    ...
    def __init__(self,
        ...
        if use_gumbel is True:
            ...
            self.unirect1 = nn.ReLU(inplace=True)
            self.unirect2 = nn.ReLU(inplace=True)
            self.unirect3 = nn.ReLU(inplace=True)

        
        self.downsample = downsample
        
        self.se = SE_Block(planes * self.expansion, r,use_gumbel=use_gumbel)
...
class SE_ResNet(BaseModule):
    ...
    def __init__(self,
        ...
        self._make_stem_layer(in_channels, stem_channels,use_gumbel=use_gumbel)
    ...
    def _make_stem_layer(self, in_channels, stem_channels,use_gumbel=False):
        if self.deep_stem:
